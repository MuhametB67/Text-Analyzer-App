...
Each task has soft + hard time limits. If a task exceeds the soft limit,
SoftTimeLimitExceeded is raised inside the task and we return a partial
"timed out" result instead of crashing the whole chord.
"""
import re
import time
import math
...
def _timeout_payload(task_name: str, worker: str) -> dict:
    """Returned when a task hits its soft time limit."""
    return {
        "task": task_name,
        "error": "timeout",
        "partial": True,
        "message": f"task exceeded soft_time_limit of {SOFT_TIME_LIMIT}s",
        "worker": worker,
    }
...
CHUNK_SIZE = 10_000  # characters


def chunk_text(text: str, chunk_size: int = CHUNK_SIZE) -> list:
    """Split text into chunks of ~chunk_size chars.

    Tries to break on sentence boundaries (. ! ?) so we don't split a word
    or sentence in half. Falls back to a hard cut if no boundary is nearby.
    """
    ...
    while start < n:
        ...
        if end < n:
            ...
            window_start = max(end - 200, start + 1)
            boundary = -1
...
def basic_stats(self, text: str) -> dict:
    try:
        ...
        return _timeout_payload("basic_stats", self.request.hostname)
...
def readability(self, text: str) -> dict:
    try:
        ...
        return _timeout_payload("readability", self.request.hostname)
...
def sentiment(self, text: str) -> dict:
    try:
        ...
        return _timeout_payload("sentiment", self.request.hostname)


@celery_app.task(name="keywords", bind=True,
                 soft_time_limit=SOFT_TIME_LIMIT, time_limit=HARD_TIME_LIMIT)
def keywords(self, text: str, top_n: int = 15) -> dict:
    """Process text in chunks. Each chunk contributes to the freq counter,
    so memory stays bounded and the worker yields between chunks (the
    SoftTimeLimitExceeded check happens between iterations).
    """
    try:
        time.sleep(0.5)
        ...
        for chunk in chunks:
            ...
            if carry and filtered:
                bigrams[f"{carry} {filtered[0]}"] += 1
            carry = filtered[-1] if filtered else carry
        ...
        top_bigrams = [
            {"phrase": p, "count": c}
            for p, c in bigrams.most_common(8)
            if c > 1
        ]
        ...
        return _timeout_payload("keywords", self.request.hostname)


@celery_app.task(name="entities", bind=True,
                 soft_time_limit=SOFT_TIME_LIMIT, time_limit=HARD_TIME_LIMIT)
def entities(self, text: str) -> dict:
    """Process text in chunks. Per-chunk regex finds get unioned (for unique
    sets like emails/urls) or summed (for counts) at the end.
    """
    try:
        time.sleep(0.4)
        ...
        COMMON = {"The", "A", "An", "And", "But", "Or", "However", "Also",
                  "Email", "Visit", "Click", "See"}

        for idx, chunk in enumerate(chunks):
            emails_set.update(re.findall(r"[\w\.-]+@[\w\.-]+\.\w+", chunk))
            urls_set.update(re.findall(r"https?://[^\s<>\"']+", chunk))
            hashtags_set.update(re.findall(r"#\w+", chunk))
            mentions_set.update(re.findall(r"(?<!\w)@\w+", chunk))
            numbers_total += len(re.findall(r"\b\d[\d,]*\.?\d*\b", chunk))
            dates_set.update(re.findall(
            ...
            proper_counter.update(p for p in proper_nouns if p not in COMMON)

        top_proper = [
            {"name": n, "count": c}
            for n, c in proper_counter.most_common(10)
            if c >= 1 and len(n) > 2
        ]
        ...
        return _timeout_payload("entities", self.request.hostname)
